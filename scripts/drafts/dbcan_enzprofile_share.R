...
mybinary <- gt_hmmerZ %>% 
  ...
  tally() %>%
  pivot_wider(id_cols = enzymeID, names_from = genus, 
              values_from=n, values_fill = list(n = 0))


head(mybinary)

# order matrix : left to right: high to low total enzyme counts 
x <- as.data.frame(mybinary)
rownames(x) <- x$enzymeID
x$enzymeID <- NULL
# order left to right in descending order 
x <- x[,names(sort(colSums(x), decreasing = TRUE))]

# subset to 20 genera that have the most (diverse) enzymatic profile
x <- x[,1:15]

head(x)

x$enzymeID <- rownames(x)

vennSets <- x %>%
  pivot_longer(cols = -enzymeID) %>%
  dplyr::select(enzymeID,name)
...
v$labels <- gsub("Prevotella","\n\nPrevotella",v$labels)
v$labels <- gsub("Blautia_A","\nBlautia_A",v$labels)


plot(v, 
     main = "How much different genera share", 
     cex.main = 1.5)
...
# other way to look at it: heatmap:
...
x
# set thresholds
x <- x[,colSums(x) < 50 & colSums(x) > 5]
x <- x[rowSums(x) < 10,]


#library(pheatmap)
pheatmap(x, fontsize = 2)
